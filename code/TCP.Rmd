...
```{r}
# logueo al api que saque desde la pagina  https://research.stlouisfed.org/useraccount/apikey
...
#vieja forma de cargar los datos
# base_manuf_us2 <- read_csv( "../data/manufacturing_production.csv")
```
...
```{r message=FALSE, warning=FALSE}
# el problema de BLS api es que te deja bajar de una decada nomas, por lo que se decide bajar e ir actualizando (joineando) el historico en nuestra base. El problema aca es que cada tanto cambian para atras la base y nosotros la tendriamos estatica REVISAR CADA TANTO Y BAJAR DE LA PAGINA MANUALMENTE PARA TENERLA BIEN
...
#Este codigo sirve para bajar alguna decada en particular
# payload <- list(
#  'seriesid'=c('CES3000000001'),
#  'startyear'=2010,
#  'endyear'=2022)
...
actualiza_employ <- distinct(actualiza_employ)
actualiza_employ <- actualiza_employ %>% 
  arrange(Year,Period)
```
...
```{r}
#serie historica
base_ipc_us2 <- read_csv("../data/cpi.csv")

# API BLS
base_ipc_us <- blsAPI('CUSR0000SA0')
base_ipc_us <- fromJSON(base_ipc_us)

actualiza_ipc <- base_ipc_us$Results$series[[1,2]]
...
# Aca unimos historica con la actualizada y eliminamos los meses duplicados
actualiza_ipc <- rbind(select(base_ipc_us2, -Label),actualiza_ipc)

actualiza_ipc <- distinct(actualiza_ipc)
actualiza_ipc <- actualiza_ipc %>% 
  arrange(Year,Period)
```
...
ipc_us <- actualiza_ipc %>% 
  mutate(mes = as.double(str_remove(actualiza_ipc$Period, "M")),
